...
* @return Transport state
*/
TypedefEnum_MasterTransportSates master_writeHoldings_RTU(uint8_t slave, uint16_t adr, uint16_t val, int timeout)
{
    MASTER_TRANSPORT_LOCK_TAKE();
    masterRxTxBuf[0] = slave;
...
TypedefEnum_MasterTransportSates master_readHoldingOs_RTU(uint8_t slave, uint16_t adr, uint16_t* out, int timeout)
{
   // TypedefEnum_MasterTransportSates retVal =
    return  master_readHoldingsOs_RTU(slave, adr, 1, out, timeout);
}
