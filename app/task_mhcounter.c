...
Typedef_MotorHoursCounterRecord time_glob;
Typedef_MotorHoursCounterRecord * time_rx;
Typedef_MotorHoursCounterRecord * tmp_records;
...
void vTask_MCHReader();
void MHC_ErrorHandler();
void MHC_search_valid_record();
bool MHC_checkRecordCRC16(Typedef_MotorHoursCounterRecord * rec);

// use block 1 as work hour counter
void vTask_MotorHoursCounter(){
  TypeDef_Inverter * inverter_now = &inverter;
  MotorControl_TypeDef * motor_now = &motor;
  ...
  if(check_mem_id == false){
    ...
    MHC_search_valid_record();
  }

  xTaskCreate(vTask_MCHReader, "", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY+1, NULL);
  ...
  while(1){
    ...
    time_glob.motorOnlineCounter += MOTOR_HOUR_COUNTER_TIME_STEP_SEC;
    ...
    volatile uint16_t crc = crc16( (uint8_t *) &time_glob, sizeof(Typedef_MotorHoursCounterRecord) );
    time_glob.crc16 = crc;
...
void vTask_MCHReader(){
  ...
  while(1){
    ...
    vGD25PageReadAsync(adr_rx, page_gd25_rx);
    time_rx = (Typedef_MotorHoursCounterRecord *) page_gd25_rx;
...
void MHC_search_valid_record(){
  while (1)
  {
    ...
    xSemaphoreGive(semaphore_MHC);
    tmp_records = (Typedef_MotorHoursCounterRecord *) page_gd25_rx;
    record_cnt = 0;
...
bool MHC_checkRecordCRC16(Typedef_MotorHoursCounterRecord * rec){
  ...
  Typedef_MotorHoursCounterRecord tmp_rec = * rec;
  tmp_rec.crc16 = 0;
  uint16_t tmp_crc = crc16( (uint8_t *) &tmp_rec, sizeof(Typedef_MotorHoursCounterRecord));
